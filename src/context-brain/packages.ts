...
// *module*, not the repo. A repo with no declared packages resolves to exactly one
...
export interface ResolvedModule {
  ...
  stack: string[];
  group: string; // hiring group (defaults to the module name) — packages sharing it share a pair
...
export function resolvePackages(brain: BrainFile): ResolvedModule[] {
...
export function packageFqid(repo: string, module?: string | null): string {
...
export function resolveGroups(brain: BrainFile): { repo: string; group: string; packages: ResolvedModule[] }[] {
  const byKey = new Map<string, { repo: string; group: string; packages: ResolvedModule[] }>();
