...
// block once RESERVED the place for the delivery metric (journey j-20260827-kj,
// mechanism out of scope) with an honest "not measured yet" placeholder (SDD
// §6.3). That mechanism now exists — the Relatório screen (/report). So this
// block stops claiming "not measured" (which would now be a lie) and instead
// links to the full report. Below, the full timeline of state changes.
  ...
  return (
    <section class="hist-metrics card pad">
      <div class="between" style={{ marginBottom: "10px" }}>
        ...
        <button type="button" class="hist-metrics-link" onClick={() => navigate("/report")}>{t("hist_metrics_link")}</button>
      </div>
