...
> **Este documento é o PORTÃO.** O coordenador (Heisenberg) mudou meu fluxo:
> entregar PRIMEIRO o mapa de IA e levá-lo ao PE **antes** de fechar o build. A
> interface já reprovou **duas vezes** (Floor original PR #22, Floor acionável PR
> #26). O problema é de **fundo**, não de mais um patch. Nada de código de UI é
> escrito antes deste mapa ser aprovado. Registrado no ledger como `redirected`.
...
## 8. Decisões em aberto para o PE

Duas escolhas genuínas de produto. Recomendo uma de cada, mas quero o aval:

- **Decisão A — o "quadro completo" (kanban) fica onde?**
  - **Recomendo:** seção recolhida dentro de "Agora" (divulgação progressiva). Menos
    telas; o que precisa de você continua no topo.
  - Alternativa: uma 4ª aba "Quadro". Ganha um lugar dedicado, custa uma tela a mais.

- **Decisão B — o toolbox (o que a equipe sabe fazer) fica onde?**
  - **Recomendo:** uma seção dentro de "Equipe" (capacidades junto de quem as usa).
  - Alternativa: junto do Glossário como "referência", tirando-o do caminho.

Também confirmo os **rótulos**: "Agora / Equipe / Histórico / Glossário / Ajustes"
— todos sem jargão. Se o PE preferir "Início" a "Agora", troco.
...
**Estado:** aguardando aprovação da IA pelo PE (via coordenador) antes de escrever
código de UI. Registrado no ledger como `blocked` apontando para este arquivo.
